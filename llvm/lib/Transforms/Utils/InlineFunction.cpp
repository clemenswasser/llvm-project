...
/// inlined at \p InlinedAt. \p IANodes caches rebuilt inlined-at chains and
/// \p InlineLocs caches the final inlined locations for this call site.
static DebugLoc
inlineDebugLoc(DebugLoc OrigDL, DILocation *InlinedAt, LLVMContext &Ctx,
               DenseMap<const MDNode *, MDNode *> &IANodes,
               DenseMap<const DILocation *, DILocation *> &InlineLocs) {
  if (auto It = InlineLocs.find(OrigDL.get()); It != InlineLocs.end())
    return It->second;

  auto IA = DebugLoc::appendInlinedAt(OrigDL, InlinedAt, Ctx, IANodes);
  DILocation *InlineLoc = DILocation::get(
      Ctx, OrigDL.getLine(), OrigDL.getCol(), OrigDL.getScope(), IA,
      OrigDL.isImplicitCode(), OrigDL->getAtomGroup(), OrigDL->getAtomRank());
  InlineLocs[OrigDL.get()] = InlineLoc;
  return InlineLoc;
}
